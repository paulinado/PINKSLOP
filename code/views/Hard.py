...
def createPage(key):
    ...
    with col3:
        if col3.button("Generate Hard Problem"):
            next_question()
    ...
    with col2:
        button = st.empty()
        click = button.button("Begin The Hard Module 🎉", on_click=click_button)
        if st.session_state.clicked:
            button.empty()
            st.progress(st.session_state.questions_answered/50, string)
            display_question()