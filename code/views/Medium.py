...
def display_question():
    ...
    st.write(question['question'].replace('$','\$'))
    ...
    if submitted: 
        if answer == question['answer']:
            ...
            form.write(question['explanation'].replace('$','\$'))
            failed_attempt()
...
def createPage(key):
    ...
    with col3:
        if col3.button("Generate Medium Problem"):
            next_question()
    ...
    with col2:
        button = st.empty()
        click = button.button("Begin The Medium Module 🎉", on_click=click_button)
        if st.session_state.clicked:
            button.empty()
            st.progress(st.session_state.questions_answered/30, string)
            display_question()